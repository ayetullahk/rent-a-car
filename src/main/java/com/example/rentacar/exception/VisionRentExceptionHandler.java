...
import com.example.rentacar.exception.message.ApiResponseError;
...
public class VisionRentExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    @ExceptionHandler(ResourceNotFoundException.class) // anotation ile exceptin yakalama
    protected ResponseEntity<Object> handleResourceNotFoundException(ResourceNotFoundException ex, WebRequest request) {//webRequest=gelen rewuest
    ...
    protected ResponseEntity<Object>handleAccessDeniedException(ConflictException ex, WebRequest request){
        ApiResponseError error=new ApiResponseError(HttpStatus.FORBIDDEN
                ,ex.getMessage(),request.getDescription(false));
        return buildResponseEntity(error);
    }
    @ExceptionHandler(AuthenticationException.class)
    protected ResponseEntity<Object>handleAuthenticationException(ConflictException ex, WebRequest request){
        ApiResponseError error=new ApiResponseError(HttpStatus.BAD_REQUEST
                ,ex.getMessage(),request.getDescription(false));
        return buildResponseEntity(error);
    }
    ...
    protected ResponseEntity<Object> handleRuntimeException(ResourceNotFoundException ex, WebRequest request) {//webRequest=gelen rewuest
        ApiResponseError error = new ApiResponseError(HttpStatus.INTERNAL_SERVER_ERROR,
                ex.getMessage(),
                request.getDescription(false));
    ...
    protected ResponseEntity<Object> handleException(ResourceNotFoundException ex, WebRequest request) {//webRequest=gelen rewuest
        ApiResponseError error = new ApiResponseError(HttpStatus.INTERNAL_SERVER_ERROR,
                ex.getMessage(),
                request.getDescription(false));
